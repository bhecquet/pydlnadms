...
import socket
import logging
from backend.pydlnadms.http import HTTPRequest, HTTPResponse, rfc1123_date
from backend.pydlnadms.server import ROOT_DESC_PATH, SERVER_FIELD
import random
import struct
import errno
import select
...
class SSDPAdvertiser(object):
    ...
    def notify_interfaces(self):
        from backend._misc.getifaddrs import getifaddrs, IFF_LOOPBACK
        for ifaddr in getifaddrs():
            if ifaddr.family == socket.AF_INET: #and not ifaddr.flags & IFF_LOOPBACK:
...
class SSDPResponder(object):
    ...
    def update_multicast_membership(self):
        from backend._misc.getifaddrs import getifaddrs
        try:
            for ifaddr in getifaddrs():
